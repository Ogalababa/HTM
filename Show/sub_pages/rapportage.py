...
def rapportage():
    ...
    if mode == 'Wekelijks rapport':
        ...
        w_storing = storing_rapport(str_week_date, 'week')
        export_as_pdf = st.sidebar.button("Download Rapport")
        if export_as_pdf:
            pdf = FPDF()
            for fig in w_speed + w_storing:
        ...
        m_speed = snelheid_rapport(str_month_date, 'maand')
        m_storing = storing_rapport(str_month_date, 'maand')

        export_as_pdf = st.sidebar.button("Download Rapport")
        if export_as_pdf:
            pdf = FPDF()
            for fig in m_speed + m_storing:
                pdf.add_page(orientation='L')
                with NamedTemporaryFile(delete=False, suffix='.png') as tmpfile:
                    pio.write_image(fig, tmpfile.name, height=500)
                    pdf.image(tmpfile.name, 10, 10)
        ...
        str_year_date.sort()
        y_speed = snelheid_rapport(str_year_date, 'jaar')
        y_storing = storing_rapport(str_year_date, 'jaar')
        export_as_pdf = st.sidebar.button("Download Rapport")
        if export_as_pdf:
            pdf = FPDF()
            for fig in y_speed + y_storing:
                pdf.add_page(orientation='L')
                with NamedTemporaryFile(delete=False, suffix='.png') as tmpfile:
                    pio.write_image(fig, tmpfile.name, height=500)
                    pdf.image(tmpfile.name, 10, 10)
