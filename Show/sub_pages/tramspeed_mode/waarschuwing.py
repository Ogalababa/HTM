...
def waarschuwing(df_all_data,speed_record_size, layout_height):
    ...
    if richting == 'rechts/links af':
        speed_counts = speed_counts[
            (speed_counts['Richting'] == ' rechts af') | (speed_counts['Richting'] == ' links af')]
    elif richting == 'recht door':
        speed_counts = speed_counts[speed_counts['Richting'] == 'Recht door']
    else:
        pass
    with col1:
        st.sidebar.metric(f'Totale records boven {select_speed} km/h:', len(speed_counts),
                          delta=f'{round(((len(speed_counts) / speed_record_size) * 100), 3)}% van totale records',
                          delta_color="normal")
